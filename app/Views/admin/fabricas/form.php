...
<div class="flex items-center gap-3 mb-6">
    ...
    <h2 class="text-[1.35rem] font-bold text-dark m-0">
        <i class="fas fa-<?= $fabrica ? 'pen' : 'plus-circle' ?> mr-2 text-rojo text-[1.05rem]"></i>
        <?= $fabrica ? 'Editar fábrica' : 'Nueva fábrica' ?>
    </h2>
</div>
...
<div class="bg-white border border-gray-100 rounded-2xl p-8 shadow-[0_1px_4px_rgba(0,0,0,0.05)] max-w-[620px]">
    <form action="<?= esc($accion) ?>" method="POST">
        ...
        <div class="text-[0.72rem] font-bold uppercase tracking-[0.9px] text-gray-400 mb-4 pb-2 border-b border-gray-100"><i class="fas fa-industry mr-1"></i>Datos de la fábrica</div>
        ...
        <div class="flex flex-wrap gap-4 mb-6">
            <div class="w-full md:w-7/12">
                ...
                <input type="text" id="nombre" name="nombre" class="<?= $inputCls ?>"
                       value="<?= esc(old('nombre', $fabrica['nombre'] ?? '')) ?>"
                       placeholder="Ej: Muebles del Sur, Maderera Norte" required
            ...
            <div class="w-full md:w-5/12">
                ...
                <input type="text" id="slug" name="slug" class="<?= $inputCls ?>"
                       value="<?= esc(old('slug', $fabrica['slug'] ?? '')) ?>"
                       placeholder="muebles-del-sur" required>
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="bg-rojo hover:bg-rojo-dark text-white px-8 py-[0.7rem] rounded-full text-[0.95rem] font-semibold inline-flex items-center gap-[0.45rem] cursor-pointer transition-colors">
                <i class="fas fa-save"></i> <?= $fabrica ? 'Guardar cambios' : 'Crear fábrica' ?>
            </button>
            <a href="<?= base_url('admin/fabricas') ?>" class="text-gray-500 hover:bg-gray-100 hover:text-gray-700 no-underline px-[1.4rem] py-[0.7rem] rounded-full text-[0.95rem] font-semibold inline-flex items-center gap-[0.4rem] border-[1.5px] border-gray-200 transition-colors">
